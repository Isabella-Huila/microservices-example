...
const TODOS_API_URL = '/api/todos/todos'
...
export default {
  ...
  methods: {
    loadTasks () {
      ...
      this.errorMessage = ''
      this.$http.get(TODOS_API_URL).then(response => {
    ...
    addTask () {
      if (this.newTask) {
        ...
        this.$http.post(TODOS_API_URL, task).then(response => {
          this.newTask = ''
          this.isProcessing = false
    ...
    removeTask (index) {
      ...
      this.$http.delete(TODOS_API_URL+ "/" + item.id).then(response => {
        this.isProcessing = false
